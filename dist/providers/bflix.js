...
source.getResource = function (movieInfo, config, callback) { return __awaiter(_this, void 0, void 0, function () {
    ...
    return __generator(this, function (_f) {
        switch (_f.label) {
            case 0:
                PROVIDER = 'BMOVIESTREAM_2';
                DOMAIN = "https://bflix.ru";
                userAgent = {};
                LINK_DETAIL = '';
                genrandom = function () {
            ...
            case 11:
                ...
                parseSearch('.item').each(function (key, item) {
                    var title = parseSearch(item).find('.info h3 a').text();
                    var year = parseSearch(item).find('.info').html();
                    year = year.match(/<span>(\d+)/i);
                    year = year ? year[1] : 0;
                    var href = parseSearch(item).find('.info h3 a').attr('href');
